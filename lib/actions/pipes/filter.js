...
function pipeFilter(ctx, cbs){
    ...
    validateConditions(ctx, conditions, {
        success: function(successCtx){
            console.log('FILTERED: '+self.cfg.target);
...
function validateConditions(ctx, conditionObj, cbs){
    if(Array.isArray(conditionObj)){ //OR conditions
        ...
        if(valid){
            ...
            if(cbs && cbs.error){
                cbs.error(new Error('NOT VALID'), ctx);
            }
        }
...
pipeFilter.flux_pipe = {
    name: 'FluxPipes:Pipes:FilterTo',
    description: 'Only passes processing to the target pipe if the filter passes',
    configs:[
        {
            name: 'conditions',
            description: 'This action will publish to cfg.target if the conditions pass and then return, otherwise will return immediately'
        }
    ]
};
